...
from utilities.proposal_utils import (
    get_corner_coords,
...
def load_features_from_npy(
    ...
    device: int,
    get_full_feat=False,
    pad_feats_up_to: Dict[str, int] = None,
...
def load_prop_model(
    device: int,
    prop_generator_model_path: str,
    pretrained_cap_model_path: str,
    max_prop_per_vid: int,
) -> tuple:
    ...
    cfg.train_meta_path = (
        "./data/train.csv"  # in the saved config it is named differently
    )
...
def load_cap_model(pretrained_cap_model_path: str, device: int) -> tuple:
    ...
    model.load_state_dict(
        cap_model_cpt["model_state_dict"]
    )  # if IncompatibleKeys - ignore
    model.eval()
...
def generate_proposals(
    ...
    # form input batch
    batch = {"feature_stacks": feature_stacks, "duration_in_secs": duration_in_secs}
    ...
    with torch.no_grad():
        for start, end, conf in proposals.squeeze():
            ...
            ints_stack = greedy_decoder(
                ...
                feature_stacks,
                cfg.max_len,
                train_dataset.start_idx,
                train_dataset.end_idx,
                train_dataset.pad_idx,
                cfg.modality,
            )
            assert (
                len(ints_stack) == 1
            ), "the func was cleaned to support only batch=1 (validation_1by1_loop)"

            # transform integers into strings
            strings = [
                train_dataset.train_vocab.itos[i] for i in ints_stack[0].cpu().numpy()
            ]
...
def which_ffprobe() -> str:
    ...
    result = subprocess.run(
        ["which", "ffprobe"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
...
def get_video_duration(path):
    ...
    cmd = (
        f"{which_ffprobe()} -hide_banner -loglevel panic"
