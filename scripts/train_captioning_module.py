...
from epoch_loops.captioning_epoch_loops import (
    greedy_decoder,
    save_model,
    training_loop,
...
def train_cap(cfg):
    ...
    if cfg.optimizer == "adam":
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), cfg.lr, cfg.momentum, weight_decay=cfg.weight_decay
        )
    ...
    for epoch in range(cfg.epoch_num):
        print(
            f"The best metrict was unchanged for {num_epoch_best_metric_unchanged} epochs."
        )
        print(
            f"Expected early stop @ {epoch+cfg.early_stop_after-num_epoch_best_metric_unchanged}"
        )
        ...
        if epoch >= cfg.one_by_one_starts_at:
            ...
            if cfg.to_log:
                ...
                TBoard.add_scalar(
                    "metrics/precision", metrics_avg["Precision"] * 100, epoch
                )
